...
func TestConsumeSink_TracksTopologyRecoveryConfiguration(t *testing.T) {
	subs := makeConsumers()
	config := consumerConfig{
		Queue:     "recoverable-queue",
		Consumer:  "recoverable-sink",
		AutoAck:   true,
		Exclusive: false,
		NoLocal:   false,
		NoWait:    false,
		Args:      Table{"x-priority": int32(5)},
	}

	subs.addSink(config.Consumer, func(Delivery) {}, config)
	if queue, ok := subs.queueForTag(config.Consumer); !ok || queue != config.Queue {
		t.Fatalf("sink recovery queue = %q, %v; want %q, true", queue, ok, config.Queue)
	}

	subs.Lock()
	got, tracked := subs.configs[config.Consumer]
	subs.Unlock()
	if !tracked || got.Consumer != config.Consumer || got.AutoAck != config.AutoAck {
		t.Fatalf("sink recovery config = %#v, %v; want %#v, true", got, tracked, config)
	}

	if !subs.cancel(config.Consumer) {
		t.Fatal("cancel did not find the recovery-tracked sink")
	}
	if _, ok := subs.queueForTag(config.Consumer); ok {
		t.Fatal("cancel left the sink recovery configuration behind")
	}
	subs.close()
}
